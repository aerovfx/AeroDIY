...
import { analyzeAerodynamics, runAerodynamicSweep, runLbmValidation, type CfdAnalysisResult, type LbmPreset, type LbmValidationResult } from "@/lib/cfd-engine";
import { CfdLbmCanvas } from "@/app/components/CfdLbmCanvas";
import { CfdGpuViewport } from "@/app/components/CfdGpuViewport";
import { buildProjectZip, partVendorOptions, type ExportConnection, type InstructionPreamble, type InstructionSection, type ProjectPart, type VendorOption } from "@/lib/project-export";
...
import { EMOTO_BIKE, buildEmotoBikeCadProject, emotoBikeElectricalConnections, emotoBikeInstructionPreamble, emotoBikeInstructionSteps, emotoBikeMechanicalConnections, emotoBikeParts } from "@/lib/emoto-bike-data";
import { BIODIESEL_REACTOR, buildBiodieselCadProject, biodieselElectricalConnections, biodieselInstructionPreamble, biodieselInstructionSteps, biodieselMechanicalConnections, biodieselParts } from "@/lib/biodiesel-reactor-data";
import { SMART_HOME, buildSmartHomeCadProject, smartHomeElectricalConnections, smartHomeInstructionPreamble, smartHomeInstructionSteps, smartHomeMechanicalConnections, smartHomeParts } from "@/lib/smart-home-data";
import { CARGO_DRONE, buildCargoDroneCadProject, cargoDroneElectricalConnections, cargoDroneInstructionPreamble, cargoDroneInstructionSteps, cargoDroneMechanicalConnections, cargoDroneParts } from "@/lib/cargo-drone-data";
import { MICRO_DRONE, buildMicroDroneCadProject, microDroneElectricalConnections, microDroneInstructionPreamble, microDroneInstructionSteps, microDroneMechanicalConnections, microDroneParts } from "@/lib/micro-drone-data";
import { EXO_SUIT, buildExoSuitCadProject, exoSuitElectricalConnections, exoSuitInstructionPreamble, exoSuitInstructionSteps, exoSuitMechanicalConnections, exoSuitParts } from "@/lib/exo-suit-data";
import { LIFT_BOOT, buildLiftBootCadProject, liftBootElectricalConnections, liftBootInstructionPreamble, liftBootInstructionSteps, liftBootMechanicalConnections, liftBootParts } from "@/lib/lift-boot-data";
import { generateDesignFromPrompt, type GeneratedDesign, type GeneratedProjectMeta } from "@/lib/design-generator";
...
import { sendProjectToPlasticity } from "@/lib/plasticity-runtime";
import { buildNativeCadArtifacts, type DiyCadArtifact } from "@/lib/freecad-runtime";
import { PurchasingView } from "@/app/components/PurchasingView";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";

type Tab = "INFO" | "UAV_LAB" | "CFD" | "PARTS" | "WIRING" | "MECH" | "INSTRUCTIONS" | "PURCHASING";
type PartView = "cards" | "table";

const tabs: Array<{ id: Tab; icon: string; label: string }> = [
  { id: "INFO", icon: "▧", label: "INFO" },
  { id: "UAV_LAB", icon: "⌁", label: "UAV LAB" },
...
const uavProjectModes = new Set<ProjectMode>(["mini", "long-range", "mother", "vtol", "observer", "ornithopter", "fpvracer", "filmingdrone", "deliverydrone", "endurance", "cargodrone", "microdrone"]);
...
export default function Home() {
  ...
  const [plasticityState, setPlasticityState] = useState<"idle" | "connecting" | "ready" | "error">("idle");
  const [plasticityMessage, setPlasticityMessage] = useState("Ready to send this feature tree to the local Plasticity kernel.");
  ...
  async function submitPrompt() {
    ...
    if (!value) return;
    if (projectMode === "generated") { runGenerate(value); return; }
    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        {projectMenuOpen && (
          <div className="project-menu">
            ...
            <span>UAV DEVELOPMENT ({projectTemplates.filter((item) => uavProjectModes.has(item.mode)).length})</span>
            <div className="project-menu-list">
              {projectTemplates.filter((item) => uavProjectModes.has(item.mode)).map((item) => (
            ...
            <details className="other-projects"><summary>OTHER DIY SYSTEMS ({projectTemplates.filter((item) => !uavProjectModes.has(item.mode)).length})</summary><div className="project-menu-list">
              {projectTemplates.filter((item) => !uavProjectModes.has(item.mode)).map((item) => <button key={item.mode} className={projectMode === item.mode ? "active" : ""} onClick={() => selectProject(item.mode)}><b>{item.title}</b><small>{item.subtitle}</small></button>)}
            </div></details>
          </div>
        )}
      ...
      <div className={`workbench ${chatCollapsed ? "chat-collapsed" : ""}`}>
        <aside className="studio-chat">
          ...
          <div className="chat-compose">
            ...
            <div className="compose-box">
              ...
              <div><button className="model-button">MCP CAD⌄</button><button className="generate-button" onClick={() => { if (prompt.trim()) runGenerate(prompt.trim()); }} disabled={!prompt.trim() || cadBusy} title="Sinh sản phẩm mới từ mô tả (qua CAD engine)" aria-label="Tạo sản phẩm mới">✨ TẠO</button><button className="send-button" onClick={submitPrompt} disabled={!prompt.trim() || cadBusy} aria-label="Gửi yêu cầu">{cadBusy ? "…" : "↗"}</button></div>
            </div>
          </div>
        </aside>
        ...
        <section className="project-stage">
          ...
          {activeTab === "UAV_LAB" && <UavLab projectName={project.name} isUav={projectMode === "generated" || uavProjectModes.has(projectMode)} componentCount={project.componentCount} total={total} validationScore={cadProject.validation.score} electricalCount={electricalConnections.length} />}
          {activeTab === "CFD" && <CfdWorkbench project={cadProject} />}
          {activeTab === "PARTS" && <PartsView projectKey={projectMode} projectName={project.name} parts={parts} componentCount={project.componentCount} connections={{ electrical: electricalConnections, mechanical: mechanicalConnections }} selectedPart={selectedPart} onSelect={(part) => { setSelectedPart(part); setPartDetailOpen(true); }} detailOpen={partDetailOpen} onBack={() => setPartDetailOpen(false)} total={total} view={partView} onView={setPartView} search={partSearch} onSearch={setPartSearch} category={partCategory} onCategory={setPartCategory} />}
          {activeTab === "PURCHASING" && <PurchasingView projectKey={projectMode} parts={parts} projectedTotal={total} />}
          ...
          {activeTab === "MECH" && <MechView key={cadProject.draftId} version={version} cadProject={cadProject} committed={committed} plasticityState={plasticityState} plasticityMessage={plasticityMessage} onCommit={commitToPlasticity} />}
...
function UavLab({ projectName, isUav, componentCount, total, validationScore, electricalCount }: { projectName: string; isUav: boolean; componentCount: number; total: number; validationScore: number; electricalCount: number }) {
  const [mission, setMission] = useState("survey");
  const [payload, setPayload] = useState(240);
  const [battery, setBattery] = useState(4200);
  const endurance = Math.max(6, Math.round((battery / 210) * (1 - payload / 2200)));
  const range = Math.max(1, Math.round(endurance * (mission === "mapping" ? .72 : mission === "inspection" ? .32 : .55) * 10) / 10);
  const reserve = Math.max(8, Math.min(35, Math.round(30 - payload / 45 + battery / 1800)));
  const readiness = Math.max(0, Math.min(100, Math.round(validationScore * .62 + reserve * .7 + (electricalCount > 4 ? 14 : 7))));
  const ready = isUav && reserve >= 20 && readiness >= 75;
  return <div className="uav-lab-page">
    <PageTitle eyebrow="AIR VEHICLE DEVELOPMENT" title="UAV LAB" meta={isUav ? "MISSION CONFIGURATOR" : "SELECT A UAV PLATFORM"} />
    {!isUav ? <section className="uav-empty"><span>⌁</span><h2>Đây chưa phải nền tảng UAV</h2><p>Chọn một mẫu trong nhóm UAV Development để cấu hình nhiệm vụ, tải trọng và ngân sách năng lượng.</p></section> : <div className="uav-lab-grid">
      <section className="mission-config">
        <header><span>01 / MISSION PROFILE</span><b>{projectName}</b></header>
        <div className="mission-options">{[["survey","SURVEY"],["mapping","MAPPING"],["inspection","INSPECTION"]].map(([id,label]) => <button key={id} className={mission === id ? "active" : ""} onClick={() => setMission(id)}><i>{id === "survey" ? "◎" : id === "mapping" ? "▦" : "⌖"}</i><b>{label}</b></button>)}</div>
        <label className="lab-slider"><span><b>PAYLOAD</b><strong>{payload} G</strong></span><input type="range" min="50" max="1200" step="10" value={payload} onChange={(event) => setPayload(Number(event.target.value))} /><small>Camera, gimbal, telemetry và thiết bị nhiệm vụ</small></label>
        <label className="lab-slider"><span><b>BATTERY CAPACITY</b><strong>{battery} MAH</strong></span><input type="range" min="850" max="12000" step="50" value={battery} onChange={(event) => setBattery(Number(event.target.value))} /><small>Dùng để ước tính sơ bộ trước khi mô phỏng vật lý</small></label>
        <div className="lab-note">Mô hình nhanh dùng cho trade-off ban đầu; luôn xác nhận bằng thrust test, log dòng điện và thử bay có kiểm soát.</div>
      </section>
      <section className="flight-envelope">
        <header><span>02 / ESTIMATED ENVELOPE</span><b className={ready ? "pass" : "review"}>{ready ? "READY FOR REVIEW" : "NEEDS REVIEW"}</b></header>
        <div className="metric-hero"><span>EST. ENDURANCE</span><b>{endurance}<small> MIN</small></b><i style={{ width: `${Math.min(100, endurance * 2.4)}%` }} /></div>
        <div className="lab-metrics"><article><span>MISSION RANGE</span><b>{range} KM</b><small>round-trip estimate</small></article><article><span>ENERGY RESERVE</span><b>{reserve}%</b><small>target ≥ 20%</small></article><article><span>READINESS</span><b>{readiness}/100</b><small>design gate score</small></article><article><span>SYSTEM</span><b>{componentCount}</b><small>components · ${total.toFixed(0)}</small></article></div>
      </section>
      <section className="uav-gates"><header>03 / PRE-FLIGHT DEVELOPMENT GATES</header>{[
        ["AIRFRAME", `${validationScore}/100 CAD validation`, validationScore >= 80],
        ["AVIONICS", `${electricalCount} verified connections`, electricalCount >= 4],
        ["ENERGY", `${reserve}% landing reserve`, reserve >= 20],
        ["PAYLOAD", `${payload} g mission payload`, payload <= 750],
      ].map(([label, detail, pass]) => <div key={String(label)}><span className={pass ? "pass" : "warn"}>{pass ? "✓" : "!"}</span><p><b>{label}</b><small>{detail}</small></p><strong>{pass ? "PASS" : "REVIEW"}</strong></div>)}</section>
    </div>}
  </div>;
}

function CfdWorkbench({ project }: { project: CadProjectResult }) {
  const [velocity, setVelocity] = useState(15);
  const [angle, setAngle] = useState(4);
  const [density, setDensity] = useState(1.225);
  const [meshPreset, setMeshPreset] = useState<"coarse" | "medium" | "fine">("medium");
  const [turbulenceModel, setTurbulenceModel] = useState<"laminar" | "k-omega-sst" | "spalart-allmaras">("k-omega-sst");
  const [spatialScheme, setSpatialScheme] = useState<"second-order-upwind" | "muscl" | "weno3">("second-order-upwind");
  const [timeIntegrator, setTimeIntegrator] = useState<"steady-pseudo-time" | "rk3">("steady-pseudo-time");
  const [cfl, setCfl] = useState(1.5);
  const [runConfig, setRunConfig] = useState<{ velocity:number; angle:number; density:number; meshPreset:"coarse" | "medium" | "fine"; turbulenceModel:"laminar" | "k-omega-sst" | "spalart-allmaras"; equationModel:"incompressible" | "euler"; analysisType:"steady" | "transient"; spatialScheme:"second-order-upwind" | "muscl" | "weno3"; timeIntegrator:"steady-pseudo-time" | "rk3"; cfl:number }>({ velocity:15, angle:4, density:1.225, meshPreset:"medium", turbulenceModel:"k-omega-sst", equationModel:"incompressible", analysisType:"steady", spatialScheme:"second-order-upwind", timeIntegrator:"steady-pseudo-time", cfl:1.5 });
  const [runState, setRunState] = useState<"ready" | "meshing" | "solving" | "complete">("ready");
  const [history, setHistory] = useState<Array<{ id: string; velocity: number; angle: number; score: number }>>([]);
  const [cfdView, setCfdView] = useState<"iso" | "top" | "front">("iso");
  const [cfdReset, setCfdReset] = useState(0);
  const [flowLayers, setFlowLayers] = useState({ domain:true, streamlines:true, particles:true, pressure:true });
  const [resultVariable, setResultVariable] = useState<"velocity" | "pressure" | "vorticity">("velocity");
  const [equationModel, setEquationModel] = useState<"incompressible" | "euler">("incompressible");
  const [analysisType, setAnalysisType] = useState<"steady" | "transient">("steady");
  const [lbmPreset, setLbmPreset] = useState<LbmPreset>("standard");
  const [gpu3dOpen, setGpu3dOpen] = useState(false);
  const [lbmPlane, setLbmPlane] = useState<"side" | "top">("side");
  const [lbmBusy, setLbmBusy] = useState(false);
  const [lbmResult, setLbmResult] = useState<LbmValidationResult | null>(null);
  const result = useMemo(() => analyzeAerodynamics(project, { velocityMs:runConfig.velocity, angleOfAttackDeg:runConfig.angle, airDensityKgM3:runConfig.density, meshPreset:runConfig.meshPreset, turbulenceModel:runConfig.turbulenceModel, spatialScheme:runConfig.spatialScheme, timeIntegrator:runConfig.timeIntegrator, cfl:runConfig.cfl }), [project, runConfig]);
  const sweep = useMemo(() => runAerodynamicSweep(project, [8, 12, 16, 20, 24], [-4, 0, 4, 8, 12, 16], runConfig.density), [project, runConfig.density]);
  const angleSweep = sweep.filter((point) => point.testVelocityMs === 16);
  const maxForce = Math.max(1, ...angleSweep.map((point) => Math.max(Math.abs(point.estimatedLiftN), point.estimatedDragN)));
  const busy = runState === "meshing" || runState === "solving";
  const workflowStage = runState === "meshing" ? 1 : runState === "solving" ? 4 : runState === "complete" ? 5 : 3;
  function runSimulation() {
    if (busy) return;
    setRunState("meshing");
    window.setTimeout(() => setRunState("solving"), 320);
    window.setTimeout(() => {
      const config = { velocity, angle, density, meshPreset, turbulenceModel, equationModel, analysisType, spatialScheme, timeIntegrator, cfl };
      const analysis = analyzeAerodynamics(project, { velocityMs:velocity, angleOfAttackDeg:angle, airDensityKgM3:density, meshPreset, turbulenceModel, spatialScheme, timeIntegrator, cfl });
      setRunConfig(config);
      setHistory((items) => [{ id:new Date().toISOString(), velocity, angle, score:analysis.score }, ...items].slice(0, 4));
      setRunState("complete");
    }, 780);
  }
  // Solver LBM chạy đồng bộ và chiếm CPU vài giây, nên nhường một nhịp cho
  // trình duyệt vẽ trạng thái "SOLVING" trước khi vào vòng lặp.
  function runLbm() {
    if (lbmBusy) return;
    setLbmBusy(true);
    window.setTimeout(() => {
      try {
        setLbmResult(runLbmValidation(project, { velocityMs:runConfig.velocity, angleOfAttackDeg:runConfig.angle, plane:lbmPlane, preset:lbmPreset }));
      } finally {
        setLbmBusy(false);
      }
    }, 60);
  }
  function downloadLbm() {
    if (!lbmResult) return;
    const content = JSON.stringify({ projectId:project.projectId, generatedAt:new Date().toISOString(), validation:lbmResult }, null, 2);
    const url = URL.createObjectURL(new Blob([content], { type:"application/json" }));
    const link = document.createElement("a"); link.href = url; link.download = `${project.projectId}-lbm-${lbmPlane}-${runConfig.velocity}ms-${runConfig.angle}deg.json`; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  function downloadResult(kind: "json" | "csv") {
    const base = `${project.projectId}-cfd-${runConfig.velocity}ms-${runConfig.angle}deg`;
    const content = kind === "json" ? JSON.stringify({ projectId:project.projectId, generatedAt:new Date().toISOString(), config:runConfig, result, sweep }, null, 2) : ["velocity_ms,aoa_deg,cd,cl,drag_n,lift_n,lift_drag,stall_risk", ...sweep.map((point) => [point.testVelocityMs,point.angleOfAttackDeg,point.dragCoefficient,point.liftCoefficient,point.estimatedDragN,point.estimatedLiftN,point.liftToDragRatio,point.stallRisk].join(","))].join("\n");
    const url = URL.createObjectURL(new Blob([content], { type:kind === "json" ? "application/json" : "text/csv" }));
    const link = document.createElement("a"); link.href = url; link.download = `${base}.${kind}`; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 1000);
  }
  return <div className="cfd-page">
    <PageTitle eyebrow="SÀNG LỌC HÌNH HỌC · CFD-LITE 2.0" title="AERODYNAMICS" meta={`${project.scene.length} GEOMETRY PRIMITIVES`} />
    <p className="cfd-heuristic-banner">
      <b>Panel này là TƯƠNG QUAN KINH NGHIỆM từ hình học, không phải nghiệm số.</b>
      Cd/Cl ở đây suy từ diện tích cản và góc tấn. Các ô mesh · residual · convergence · CFL là
      <i> giá trị minh hoạ theo preset</i>, không sinh ra từ một lần giải thật — chúng không đổi theo vận tốc hay góc tấn.
      Solver giải thật là <b>CFD-LBM</b> ở cột kết quả và hầm gió phía dưới trang.
    </p>
    <nav className="cfd-workflow" aria-label="CFD workflow">{[["GEOMETRY","CAD geometry"],["GRID","Volume mesh"],["EQUATION","Flow model"],["BOUNDARY","BC setup"],["SOLVE","Iterations"],["POST","Flow fields"]].map(([label,detail], index) => <div key={label} className={`${index < workflowStage ? "done" : index === workflowStage ? "active" : ""}`}><span>{index < workflowStage ? "✓" : index + 1}</span><p><b>{label}</b><small>{detail}</small></p></div>)}</nav>
    <div className="cfd-commandbar"><div className="case-name"><span>CASE</span><b>{project.projectId.toUpperCase()}</b><small>External aerodynamics · 3D</small></div><div className="command-tools"><button>▣ MESH</button><button>⌁ INITIALIZE</button><button className={`primary ${busy ? "running" : ""}`} onClick={runSimulation} disabled={busy}>{runState === "meshing" ? "MESHING…" : runState === "solving" ? "SOLVING…" : "▶ RUN SOLVER"}</button></div><div className="solver-state"><i className={busy ? "running" : result.solver.converged ? "ready" : ""} /><span><small>STATUS · MINH HOẠ</small><b>{busy ? runState.toUpperCase() : result.solver.converged ? "CONVERGED" : "READY"}</b></span><span><small>RESIDUAL · PRESET</small><b>{result.solver.finalResidual.toExponential(1)}</b></span></div></div>
    <div className="cfd-workbench">
      <aside className="cfd-settings"><header>SIMULATION SETUP</header>
        <label><span>FLOW VELOCITY <b>{velocity} m/s</b></span><input type="range" min="2" max="40" step="1" value={velocity} onChange={(event) => setVelocity(Number(event.target.value))} /></label>
        <label><span>ANGLE OF ATTACK <b>{angle}°</b></span><input type="range" min="-10" max="24" step="1" value={angle} onChange={(event) => setAngle(Number(event.target.value))} /></label>
        <label><span>AIR DENSITY <b>{density.toFixed(3)} kg/m³</b></span><select value={density} onChange={(event) => setDensity(Number(event.target.value))}><option value="1.225">SEA LEVEL · 15°C</option><option value="1.112">1,000 M ALTITUDE</option><option value="1.007">2,000 M ALTITUDE</option><option value="0.909">3,000 M ALTITUDE</option></select></label>
        <label><span>MESH RESOLUTION <b>{meshPreset.toUpperCase()}</b></span><select value={meshPreset} onChange={(event) => setMeshPreset(event.target.value as typeof meshPreset)}><option value="coarse">COARSE · QUICK</option><option value="medium">MEDIUM · DEFAULT</option><option value="fine">FINE · VALIDATION</option></select></label>
        <label><span>TURBULENCE <b>{turbulenceModel.toUpperCase()}</b></span><select value={turbulenceModel} onChange={(event) => setTurbulenceModel(event.target.value as typeof turbulenceModel)}><option value="laminar">LAMINAR</option><option value="k-omega-sst">K-OMEGA SST</option><option value="spalart-allmaras">SPALART–ALLMARAS</option></select></label>
        <label><span>EQUATION MODEL <b>{equationModel === "incompressible" ? "NAVIER–STOKES" : "EULER"}</b></span><select value={equationModel} onChange={(event) => setEquationModel(event.target.value as typeof equationModel)}><option value="incompressible">INCOMPRESSIBLE VISCOUS</option><option value="euler">COMPRESSIBLE INVISCID · PRESET</option></select></label>
        <label><span>ANALYSIS TYPE <b>{analysisType.toUpperCase()}</b></span><select value={analysisType} onChange={(event) => setAnalysisType(event.target.value as typeof analysisType)}><option value="steady">STATIONARY / STEADY</option><option value="transient">TIME-DEPENDENT</option></select></label>
        <label><span>SPATIAL SCHEME <b>{spatialScheme.toUpperCase()}</b></span><select value={spatialScheme} onChange={(event) => setSpatialScheme(event.target.value as typeof spatialScheme)}><option value="second-order-upwind">2ND-ORDER UPWIND</option><option value="muscl">MUSCL RECONSTRUCTION</option><option value="weno3">WENO3 · COMPRESSIBLE</option></select></label>
        <label><span>TIME INTEGRATION <b>{timeIntegrator === "rk3" ? "RK3" : "PSEUDO-TIME"}</b></span><select value={timeIntegrator} onChange={(event) => { const value = event.target.value as typeof timeIntegrator; setTimeIntegrator(value); setCfl(value === "rk3" ? .7 : 1.5); }}><option value="steady-pseudo-time">STEADY PSEUDO-TIME</option><option value="rk3">SSP RUNGE–KUTTA 3</option></select></label>
        <label><span>CFL NUMBER <b>{cfl.toFixed(1)}</b></span><input type="range" min="0.1" max="5" step="0.1" value={cfl} onChange={(event) => setCfl(Number(event.target.value))} /></label>
        <div className="boundary-conditions"><b>BOUNDARY CONDITIONS</b><span><i className="inlet" />INLET <strong>VELOCITY · {velocity} M/S</strong></span><span><i className="outlet" />OUTLET <strong>PRESSURE · 0 PA</strong></span><span><i className="wall" />OBJECT <strong>NO-SLIP WALL</strong></span><span><i className="farfield" />DOMAIN <strong>FAR-FIELD / SLIP</strong></span></div>
        <div className="domain-stats"><span>REFERENCE AREA <b>{result.referenceAreaM2} m²</b></span><span>DYNAMIC PRESSURE <b>{result.dynamicPressurePa} Pa</b></span><span>REYNOLDS NUMBER <b>{result.reynoldsNumber.toLocaleString()}</b></span></div>
        {history.length > 0 && <div className="cfd-history"><b>RECENT RUNS</b>{history.map((item) => <span key={item.id}>{item.velocity} m/s · {item.angle}° <strong>{item.score}/100</strong></span>)}</div>}
      </aside>
      <section className="flow-view"><div className="viewport-toolbar"><div><button>⌖ SELECT</button><button>↻ ORBIT</button><button>⌕ ZOOM</button><button>⊙ PROBE</button></div><label><span>RESULT</span><select value={resultVariable} onChange={(event) => setResultVariable(event.target.value as typeof resultVariable)}><option value="velocity">VELOCITY MAGNITUDE</option><option value="pressure">STATIC PRESSURE</option><option value="vorticity">VORTICITY MAGNITUDE</option></select></label></div><div className="flow-field"><div className="cfd-model-stage"><CadViewport sceneSpec={project.scene} view={cfdView} exploded={false} resetToken={cfdReset} pitchDeg={runConfig.angle} flowDomain={{ enabled:true, velocityMs:runConfig.velocity, showDomain:flowLayers.domain, showStreamlines:flowLayers.streamlines, showParticles:flowLayers.particles, particleCount:420, showPressure:flowLayers.pressure }} /></div><div className="cfd-view-controls"><span>3D FLOW DOMAIN · AOA {runConfig.angle}°</span><div><button className={cfdView === "iso" ? "active" : ""} onClick={() => setCfdView("iso")}>ISO</button><button className={cfdView === "top" ? "active" : ""} onClick={() => setCfdView("top")}>TOP</button><button className={cfdView === "front" ? "active" : ""} onClick={() => setCfdView("front")}>FRONT</button><button onClick={() => setCfdReset((value) => value + 1)}>RESET</button></div></div><div className="cfd-layer-controls"><button className={flowLayers.domain ? "active" : ""} onClick={() => setFlowLayers((value) => ({ ...value, domain:!value.domain }))}>□ DOMAIN</button><button className={flowLayers.streamlines ? "active" : ""} onClick={() => setFlowLayers((value) => ({ ...value, streamlines:!value.streamlines }))}>≋ STREAMLINES</button><button className={flowLayers.particles ? "active" : ""} onClick={() => setFlowLayers((value) => ({ ...value, particles:!value.particles }))}>∙ PARTICLES</button><button className={flowLayers.pressure ? "active" : ""} onClick={() => setFlowLayers((value) => ({ ...value, pressure:!value.pressure }))}>◉ PRESSURE</button></div><div className="result-legend"><b>{resultVariable === "velocity" ? "Velocity [m/s]" : resultVariable === "pressure" ? "Pressure [Pa]" : "Vorticity [1/s]"}</b><span>{resultVariable === "velocity" ? (runConfig.velocity * 1.6).toFixed(1) : resultVariable === "pressure" ? result.dynamicPressurePa.toFixed(0) : "12.0"}</span><i /><span>{resultVariable === "velocity" ? (runConfig.velocity * .8).toFixed(1) : "0.0"}</span><span>{resultVariable === "velocity" ? "0.0" : resultVariable === "pressure" ? (-result.dynamicPressurePa).toFixed(0) : "-12.0"}</span></div><div className="flow-boundary inlet">INLET<br/><b>{runConfig.velocity} M/S</b></div><div className="flow-boundary outlet">OUTLET<br/><b>0 PA</b></div><div className="flow-vector lift">L {result.estimatedLiftN} N</div><div className="flow-vector drag">D {result.estimatedDragN} N</div><div className="cfd-model-caption">DRAG TO ORBIT · SCROLL TO ZOOM · {project.metrics.dimensionsMm.map(Math.round).join(" × ")} MM</div></div>
        <div className="aero-kpis"><article><span>DRAG COEFFICIENT</span><b>C<small>d</small> {result.dragCoefficient}</b></article><article><span>LIFT COEFFICIENT</span><b>C<small>l</small> {result.liftCoefficient}</b></article><article><span>LIFT / DRAG</span><b>{result.liftToDragRatio}</b></article><article><span>STALL RISK</span><b className={result.stallRisk}>{result.stallRisk.toUpperCase()}</b></article></div>
      </section>
      <section className="cfd-chart"><header><span>AOA SWEEP @ 16 M/S</span><small>LIFT / DRAG FORCE</small></header><div className="bar-chart">{angleSweep.map((point) => <div key={point.angleOfAttackDeg}><div className="bars"><i className="lift-bar" style={{ height:`${Math.abs(point.estimatedLiftN) / maxForce * 100}%` }} title={`Lift ${point.estimatedLiftN} N`} /><i className="drag-bar" style={{ height:`${point.estimatedDragN / maxForce * 100}%` }} title={`Drag ${point.estimatedDragN} N`} /></div><b>{point.angleOfAttackDeg}°</b></div>)}</div><footer><span><i className="lift-key" /> LIFT</span><span><i className="drag-key" /> DRAG</span></footer></section>
      <section className="cfd-results"><header><span>VALIDATION GATES</span><b>{result.score}/100</b></header><div className="solver-summary"><span>MESH <b>{result.mesh.estimatedCells.toLocaleString()} CELLS</b></span><span>QUALITY <b>{result.mesh.minimumQuality}</b></span><span>ZONES / PARTS <b>{result.mesh.zones} / {result.mesh.partitions}</b></span><span>RESIDUAL <b>{result.solver.finalResidual.toExponential(1)}</b></span><span>SCHEME <b>{result.solver.spatialScheme.toUpperCase()}</b></span><span>CFL <b>{result.solver.cfl}</b></span></div><div className="residual-plot"><header><span>CONVERGENCE HISTORY · MINH HOẠ</span><b>LOG RESIDUAL</b></header><div>{result.solver.residualHistory.map((value,index) => <i key={index} style={{ height:`${Math.max(4, Math.min(100, (-Math.log10(value) / 5) * 100))}%` }} title={`Iteration ${Math.round(index * result.solver.iterations / 11)} · ${value.toExponential(2)}`} />)}</div><footer><span>0</span><span>{result.solver.iterations} ITER</span></footer></div>{result.checks.map((check) => <article key={check.id}><span className={check.passed ? "pass" : "fail"}>{check.passed ? "✓" : "!"}</span><p><b>{check.label}</b><small>{check.note}</small></p><strong>{check.value}</strong></article>)}<section className="lbm-gate">
      <header><span>CFD-LBM · SOLVER GIẢI THẬT</span>{lbmResult && <b className={lbmResult.passed ? "pass" : "review"}>{lbmResult.passed ? "PASS" : "REVIEW"} · {lbmResult.score}/100</b>}</header>
      <div className="lbm-gate-setup">
        <label><span>MẶT PHẲNG</span><select value={lbmPlane} onChange={(event) => setLbmPlane(event.target.value as typeof lbmPlane)}><option value="side">MẶT CẠNH · X–Y</option><option value="top">MẶT BẰNG · X–Z</option></select></label>
        <label><span>ĐỘ PHÂN GIẢI</span><select value={lbmPreset} onChange={(event) => setLbmPreset(event.target.value as LbmPreset)}><option value="quick">QUICK · ~0.6s</option><option value="standard">STANDARD · ~1.5s</option><option value="fine">FINE · ~5s</option></select></label>
        <button className={`lbm-run ${lbmBusy ? "running" : ""}`} onClick={runLbm} disabled={lbmBusy}>{lbmBusy ? "ĐANG GIẢI…" : "▶ RUN LBM"}</button>
      </div>
      {lbmResult ? <>
        <div className="lbm-gate-grid">
          <span>Cd ĐO ĐƯỢC<b>{lbmResult.measured.dragCoefficient} ± {lbmResult.measured.dragCoefficientRms}</b></span>
          <span>{lbmResult.plane === "top" ? "Cy NGANG" : "Cl NÂNG"}<b>{lbmResult.measured.liftCoefficient}</b></span>
          <span>LỰC CẢN<b>{lbmResult.measured.estimatedDragN} N</b></span>
          <span>{lbmResult.plane === "top" ? "LỰC NGANG" : "LỰC NÂNG"}<b>{lbmResult.measured.estimatedLiftN} N</b></span>
          <span>Re THỰC / MÔ PHỎNG<b>{lbmResult.reynolds.physical.toLocaleString()} / {lbmResult.reynolds.simulated.toLocaleString()}</b></span>
          <span>D · CHẮN KÊNH<b>{lbmResult.lattice.characteristicCells} ô · {(lbmResult.report.obstacle.blockageRatio * 100).toFixed(1)}%</b></span>
          <span>DIỆN TÍCH CẢN<b>{lbmResult.geometry.frontalAreaM2} m²</b></span>
          <span>SHEDDING<b>{lbmResult.measured.sheddingDetected ? `St ${lbmResult.measured.strouhal} · ${lbmResult.measured.sheddingHz} Hz` : "KHÔNG PHÁT HIỆN"}</b></span>
        </div>
        <p className="lbm-gate-regime"><b>{lbmResult.regime.label}</b>{lbmResult.regime.interpretation}<small>{lbmResult.regime.benchmark}</small></p>
        <div className="lbm-gate-checks">{lbmResult.checks.map((check) => <article key={check.id}><span className={check.passed ? "pass" : "fail"}>{check.passed ? "✓" : "!"}</span><p><b>{check.label}</b><small>{check.note}</small></p><strong>{check.value}</strong></article>)}</div>
        <p className="lbm-gate-cross"><b>ĐỐI CHIẾU CFD-LITE</b>Lực cản giải tích {lbmResult.crossCheck.analyticDragN} N so với {lbmResult.measured.estimatedDragN} N đo được ({lbmResult.crossCheck.deltaPct >= 0 ? "+" : ""}{lbmResult.crossCheck.deltaPct}% · {lbmResult.crossCheck.agreement.toUpperCase()}).<small>{lbmResult.crossCheck.note}</small></p>
        {lbmResult.geometry.skippedPrimitives.length > 0 && <p className="lbm-gate-skipped">{lbmResult.geometry.usedPrimitives} khối vào lưới ({lbmResult.geometry.cellSizeMm} mm/ô); {lbmResult.geometry.skippedPrimitives.length} chi tiết nhỏ hơn cỡ ô bị loại: {lbmResult.geometry.skippedPrimitives.slice(0, 6).join(", ")}{lbmResult.geometry.skippedPrimitives.length > 6 ? "…" : ""}</p>}
        <div className="lbm-gate-export"><button onClick={downloadLbm}>⇩ LBM JSON</button></div>
        <p className="cfd-warning">{lbmResult.disclaimer}</p>
      </> : <p className="lbm-gate-empty">Cổng CFD-LITE ở trên là tương quan kinh nghiệm từ hình học. Chạy RUN LBM để giải Navier–Stokes 2D trên bóng chiếu hình học CAD và ĐO Cd/Cl bằng momentum-exchange, rồi đối chiếu hai kết quả.</p>}
    </section><div className="cfd-export"><button onClick={() => downloadResult("csv")}>⇩ SWEEP CSV</button><button onClick={() => downloadResult("json")}>⇩ FULL JSON</button></div><p className="cfd-warning">{result.disclaimer}</p></section>
    </div>
    <CfdLbmCanvas scene={project.scene} projectId={project.projectId} angleOfAttackDeg={runConfig.angle} velocityMs={runConfig.velocity} />
    {gpu3dOpen
      ? <CfdGpuViewport scene={project.scene} projectId={project.projectId} angleOfAttackDeg={runConfig.angle} velocityMs={runConfig.velocity} />
      : <div className="cfd3d-opener"><button onClick={() => setGpu3dOpen(true)}>⬢ MỞ HẦM GIÓ 3D · WEBGPU</button><span>Solver D3Q19 chạy trên GPU quanh mesh three.js thật của dự án — cần trình duyệt hỗ trợ WebGPU, chiếm ~250 MB VRAM khi mở.</span></div>}
  </div>;
}

function InfoView({ project, cadProject, total, version, committed, toolRuns, electricalCount, onOpenMech }: { project: typeof miniProject | typeof LONG_RANGE_UAV | typeof MOTHER_UAV | typeof USV_PROJECT | typeof VTOL_DRONE | typeof OBSERVER_MULTIROTOR | typeof ORNITHOPTER | typeof MOBILE_ROBOT | typeof HUMANOID_ROBOT | typeof ROBODOG | typeof SUBMARINE | typeof RC_BOAT | typeof WIND_HARVESTER | typeof SCARA_ARM | typeof FPV_RACER | typeof PLASMA_THRUSTER | typeof WALLE_ROBOT | typeof GARDEN_IRRIGATION | typeof FILMING_DRONE | typeof COMPANION_BOT | typeof AR_GLASSES | typeof PRINTER_3D | typeof DELIVERY_DRONE | typeof ENDURANCE_DRONE | typeof CNC_MILL | typeof LARGE_PRINTER | typeof CYBER_MULTITOOL | typeof EMOTO_BIKE | typeof BIODIESEL_REACTOR | typeof SMART_HOME | typeof CARGO_DRONE | typeof MICRO_DRONE | typeof EXO_SUIT | typeof LIFT_BOOT | GeneratedProjectMeta; cadProject: CadProjectResult; total: number; version: number; committed: boolean; toolRuns: typeof initialToolRuns; electricalCount: number; onOpenMech: () => void }) {
  const outputs = project.key === "generated" ? ((project as GeneratedProjectMeta).notes ?? []).slice(0, 3).concat([`${cadProject.scene.length} khối mesh · ${cadProject.metrics.primitiveCount} primitive`]).slice(0, 3) : project.key === "exosuit" ? ["Khung nhôm với khớp in carbon-PETG và actuator PA-14 gối/khuỷu", "Pin LiFePO4 24V + BMS, cảm biến áp lực chân điều khiển", "Camera nhiệt, cảm biến khí, mic và HUD pico — đã loại hệ vũ khí"] : project.key === "liftboot" ? ["ESP32 + hai driver DRV8871 và hai bơm thuỷ lực DC", "Bốn xi-lanh 3\" (gót + mũi) giữa hai tấm đế", "Reservoir, van một chiều, pin 3S — cảnh báo kẹp/áp lực"] : project.key === "smarthome" ? ["Hub Raspberry Pi 4 với Zigbee và ba node ESP32", "Cảm biến chuyển động, khí hậu, cửa và ánh sáng đa phòng", "Điều khiển đèn LED, rèm cửa, ổ cắm và màn cảm ứng"] : project.key === "cargodrone" ? ["Pixhawk 4 Mini, Here3 GPS và module nguồn PM07", "Bốn motor F80 + ESC 55A trên khung carbon 6S", "Vòng LiDAR bốn cảm biến, FPV và geofence/failsafe"] : project.key === "microdrone" ? ["ESP32-C3 Super Mini và IMU MPU6050", "Bốn motor coreless qua MOSFET ESC", "Pin 1S, khung in 3D — tổng ~ $30"] : project.key === "emotobike" ? ["Pin 72V Li-ion, Orion BMS và Sevcon Gen4 controller", "Khung thép, giảm xóc MX và phanh thuỷ lực trước/sau", "Dash KOSO, đèn LED và bộ thân in 3D (chạy off-road/sân kín)"] : project.key === "biodiesel" ? ["Arduino Mega, relay 16 kênh và hai SSR cho heater", "Bơm định lượng, van solenoid và bình phản ứng inox 3L", "Cảm biến pH/nhiệt/lưu lượng/mức — cảnh báo methanol/lye, PPE"] : project.key === "cncmill" ? ["Arduino Uno + GRBL shield và ba driver A4988", "Ba NEMA 17 dẫn động trục vít, spindle 500W", "Khung 2040, ray tuyến tính MGN12 và công tắc hành trình"] : project.key === "largeprinter" ? ["BigTreeTech SKR 2 và năm driver TMC2209 êm ái", "Chuyển động dual-Z, BLTouch và bàn nhiệt 300mm", "Khung 2040/2020, ray MGN12 và cấp nguồn 24V Mean Well"] : project.key === "cybertool" ? ["STM32WB55, LCD 128x64 và touch/nút điều hướng", "Sub-GHz, NFC, RFID 125kHz và IR học/phát lại", "Pin LiPo 2000mAh, sạc USB-C — chỉ dùng hợp pháp trên thiết bị của bạn"] : project.key === "endurance" ? ["Thân cánh liền khối và hai tractor motor 8S", "FC H7, GPS và quản lý năng lượng tầm xa", "LiDAR, mmWave radar và điều hướng ngày/đêm"] : project.key === "deliverydrone" ? ["Pixhawk, GPS và telemetry 433MHz", "Bốn motor + ESC và giám sát FPV", "Cơ cấu thả gói servo có giám sát (<1kg)"] : project.key === "printer3d" ? ["Arduino Mega + RAMPS và bốn driver A4988", "Bốn NEMA 17, hotend MK8 và bàn nhiệt", "Khung 2020, chuyển động GT2 và kính in"] : project.key === "arglasses" ? ["ESP32-S3, ESP32-CAM và OLED phóng đại", "Head tracking IMU, mic và loa", "Nguồn Li-ion, boost và khung đeo"] : project.key === "companion" ? ["ESP32-S3 và màn TFT 2.4 inch", "Cảm biến chuyển động và môi trường", "Vỏ in 3D với tai mèo và đuôi"] : project.key === "filmingdrone" ? ["Bốn F80 + ESC 4-in-1 trên khung 7-inch", "FC H7, GPS và FPV analog", "GoPro Hero 11 trên mount chống rung"] : project.key === "garden" ? ["ESP32, relay 6 kênh và sáu van điện", "Cảm biến ẩm đất theo từng vùng", "Bơm, manifold PVC và tưới nhỏ giọt"] : project.key === "walle" ? ["RPi 5 + ESP32, thị giác và giọng nói", "Dẫn động xích và tránh vật cản", "Đầu/tay servo biểu cảm sinh động"] : project.key === "plasma" ? ["Vòng siêu dẫn và nguồn xung công suất lớn", "Xenon injector và nozzle từ tính", "Avionics rad-hard, star tracker và cảnh báo HV/cryo"] : project.key === "fpvracer" ? ["Bốn motor + ESC DShot trên khung carbon 5-inch", "FC H7 tích hợp IMU/baro/mag và GPS", "FPV analog, VTX và bố trí CG cho pin 6S"] : project.key === "scara" ? ["Ba khớp servo (base/shoulder/elbow) + gripper", "Động học SCARA và bố trí tầm với", "ESP32, nguồn 5V và trình tự gắp đặt"] : project.key === "windharvester" ? ["Generator BLDC qua chỉnh lưu 3 pha", "MPPT nạp vào bank siêu tụ", "ESP32, cảm biến công suất và anemometer"] : project.key === "rcboat" ? ["Động cơ brushless, chân vịt và bánh lái servo", "Tời servo thả/thu camera trên dây tether", "ESP32, RC receiver và FPV video link"] : project.key === "submarine" ? ["Sáu thruster: bốn ngang, hai đứng", "Cảm biến độ sâu Bar30, IMU và camera", "Thân acrylic kín nước và trim ballast"] : project.key === "robodog" ? ["Mười hai khớp BLDC (3 DOF mỗi chân) trên CAN", "RPi + ESP32 điều khiển cấp cao và thời gian thực", "Khung nhôm V-slot, chân TPU và IMU cân bằng"] : project.key === "humanoid" ? ["Khung nhôm 170cm với khớp Dynamixel", "Jetson Orin Nano, RGB-D, IMU và cảm biến xúc giác", "Hệ biểu cảm khuôn mặt và cảm biến áp lực bàn chân"] : project.key === "mobilerobot" ? ["STM32 + ESP32-CAM, hai motor có encoder", "Dò line, ToF tránh vật cản và IMU", "Cảm biến môi trường, OLED, RTC và log thẻ SD"] : project.key === "ornithopter" ? ["Cơ cấu vỗ cánh mô phỏng dragonfly, hai motor micro", "Airframe siêu nhẹ với spar carbon và màng cánh", "FPV camera, receiver và trim tâm trọng lực"] : project.key === "observer" ? ["Bốn ESC telemetry và bố trí động cơ cân bằng", "Gimbal 2 trục ổn định cho camera quan sát", "Pixhawk, GPS, telemetry 900MHz và video link"] : project.key === "vtol" ? ["Hệ động lực lai: bốn rotor lift và một motor cruise", "Bố trí CG cho pin 6S và chuyển tiếp hover ↔ cruise", "Pixhawk, SBC tự hành, GPS, camera và lidar khảo sát"] : project.key === "usv" ? ["Nguồn 48V cách ly, solar MPPT và hai waterjet điện", "RTK, radar, AIS, lidar và COLREG-aware navigation", "Sonar đa tia, chất lượng nước và thử nghiệm thủy động lực"] : project.key === "mother" ? ["Nguồn cách ly cho động cơ, avionics và sáu dock sạc", "Cân bằng tàu mẹ với rack UAV con và pin 12S", "Geofence, Remote ID, failsafe và kiểm thử docking an toàn"] : project.key === "long-range" ? ["Hệ động lực fixed-wing và quản lý năng lượng tầm xa", "Bố trí CG cho pin Li-ion dung lượng lớn", "ArduPilot, GPS, airspeed và radio telemetry"] : ["Phân phối nguồn LiPo và điều khiển bốn ESC", "Cân bằng khối lượng và bố trí tâm trọng lực", "Cấu hình failsafe và kiểm thử động cơ an toàn"];
  const engineeringOutputs = project.key === "emotobike" ? ["Pin 72V, smart BMS và rear hub BLDC", "ESP32 xử lý throttle, brake và wheel-speed sensor", "LCD telemetry, đèn LED và khung MX off-road"] : outputs;
  return <div className="info-page">
    ...
    <div className="info-grid">
      ...
      <section className="learning-panel"><span className="section-kicker">ENGINEERING OUTPUTS</span>{engineeringOutputs.map((output, index) => <div key={output}><b>0{index + 1}</b><p>{output}</p></div>)}</section>
    </div>
  </div>;
}

function PartsView({ projectKey, projectName, parts, componentCount, connections, selectedPart, onSelect, detailOpen, onBack, total, view, onView, search, onSearch, category, onCategory }: { projectKey: string; projectName: string; parts: ProjectPart[]; componentCount: number; connections: { electrical: ExportConnection[]; mechanical: ExportConnection[] }; selectedPart: ProjectPart; onSelect: (part: ProjectPart) => void; detailOpen: boolean; onBack: () => void; total: number; view: PartView; onView: (view: PartView) => void; search: string; onSearch: (value: string) => void; category: string; onCategory: (value: string) => void }) {
  ...
  const refreshSources = (partId: string) => {
    setRefreshingPart(partId);
    fetch(`/api/vendor-prices?projectKey=${encodeURIComponent(projectKey)}&partId=${encodeURIComponent(partId)}`)
      .then((response) => response.json())
      .then((payload) => setCuratedCounts((counts) => ({ ...counts, [partId]: Array.isArray(payload.vendorPrices) ? payload.vendorPrices.length : 0 })))
      .catch(() => {})
      .finally(() => setRefreshingPart(""));
  };
  if (detailOpen) return <PartDetail projectKey={projectKey} projectName={projectName} parts={parts} connections={connections} part={selectedPart} onBack={onBack} onSelect={onSelect} />;
...
function PartDetail({ projectKey, projectName, parts, connections: projectConnections, part, onBack, onSelect }: { projectKey: string; projectName: string; parts: ProjectPart[]; connections: { electrical: ExportConnection[]; mechanical: ExportConnection[] }; part: ProjectPart; onBack: () => void; onSelect: (part: ProjectPart) => void }) {
  const [curatedVendors, setCuratedVendors] = useState<VendorOption[]>([]);
  const [newVendorName, setNewVendorName] = useState("");
  const [newVendorUrl, setNewVendorUrl] = useState("");
  const [newVendorPrice, setNewVendorPrice] = useState("");
  const [savingVendor, setSavingVendor] = useState(false);
  const [vendorSaveError, setVendorSaveError] = useState("");
  const loadVendorPrices = useCallback(() => fetch(`/api/vendor-prices?projectKey=${encodeURIComponent(projectKey)}&partId=${encodeURIComponent(part.id)}`)
    .then((response) => response.json())
    .then((payload) => setCuratedVendors(Array.isArray(payload.vendorPrices) ? payload.vendorPrices.map((row: { vendorName: string; url: string; price: number; currency: string; inStock: boolean; note: string }) => ({ name: row.vendorName, url: row.url, price: row.price, currency: row.currency, inStock: row.inStock, note: row.note })) : []))
    .catch(() => setCuratedVendors([])), [projectKey, part.id]);
  useEffect(() => { loadVendorPrices(); }, [loadVendorPrices]);
  const vendorOptions = useMemo(() => {
    const merged = new Map<string, VendorOption>();
    partVendorOptions(part).forEach((option) => merged.set(option.name, option));
    curatedVendors.forEach((option) => merged.set(option.name, option));
    return Array.from(merged.values()).sort((left, right) => left.price - right.price);
  }, [part, curatedVendors]);
  const addVendorPrice = () => {
    const price = Number(newVendorPrice);
    if (!newVendorName.trim() || !Number.isFinite(price) || price < 0) return;
    setSavingVendor(true);
    setVendorSaveError("");
    fetch("/api/vendor-prices", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ projectKey, partId: part.id, vendorName: newVendorName.trim(), url: newVendorUrl.trim(), price }),
    }).then(async (response) => {
      if (!response.ok) {
        const payload = await response.json().catch(() => ({}));
        throw new Error(payload.error || "Không lưu được giá.");
      }
      await loadVendorPrices();
      setNewVendorName(""); setNewVendorUrl(""); setNewVendorPrice("");
    }).catch((error) => setVendorSaveError(error instanceof Error ? error.message : "Không lưu được giá."))
  ...
  return <div className="part-detail">
    ...
    <section className="part-detail-section"><div className="detail-section-head"><h2>SOURCING OPTIONS</h2><span>{vendorOptions.length} NGUỒN · GIÁ CÓ THỂ THAY ĐỔI</span></div><div className="source-grid">
      {vendorOptions.map((option) => <a key={option.name} className="source-card" href={option.url || "#"} target={option.url ? "_blank" : undefined} rel="noreferrer"><div><b>{option.name}</b><p>{option.note || (option.inStock === false ? "Hết hàng" : "")}</p></div><strong>${option.price.toFixed(2)}</strong></a>)}
      <div className="source-card"><div><b>PROJECT ESTIMATE</b><p>{part.qty} × ${part.price.toFixed(2)} for this build</p></div><strong>${(part.qty * part.price).toFixed(2)}</strong></div>
    </div>
    <form className="add-vendor-form" onSubmit={(event) => { event.preventDefault(); addVendorPrice(); }}>
      <input placeholder="Nhà cung cấp" value={newVendorName} onChange={(event) => setNewVendorName(event.target.value)} />
      <input placeholder="Đường dẫn (tuỳ chọn)" value={newVendorUrl} onChange={(event) => setNewVendorUrl(event.target.value)} />
      <input placeholder="Giá USD" type="number" min="0" step="0.01" value={newVendorPrice} onChange={(event) => setNewVendorPrice(event.target.value)} />
      <button type="submit" disabled={savingVendor}>{savingVendor ? "Đang lưu…" : "+ Thêm giá"}</button>
    </form>
    {vendorSaveError && <p className="vendor-save-error">{vendorSaveError}</p>}
    </section>
...
function MechView({ version, cadProject, committed, plasticityState, plasticityMessage, onCommit }: { version: number; cadProject: CadProjectResult; committed: boolean; plasticityState: "idle" | "connecting" | "ready" | "error"; plasticityMessage: string; onCommit: () => void }) {
  const [view, setView] = useState<"iso" | "top" | "front">("iso");
  const [exploded, setExploded] = useState(false);
  const [resetToken, setResetToken] = useState(0);
  const [nativeState, setNativeState] = useState<"idle" | "building" | "ready" | "error">("idle");
  const [nativeMessage, setNativeMessage] = useState("Build editable FCStd plus STEP and STL with the local OpenCascade kernel.");
  const [nativeArtifacts, setNativeArtifacts] = useState<DiyCadArtifact[]>([]);
  async function buildNativeCad() {
    if (nativeState === "building") return;
    setNativeState("building");
    setNativeMessage("FreeCAD/OpenCascade is rebuilding the validated feature tree…");
    setNativeArtifacts([]);
    try {
      const result = await buildNativeCadArtifacts(cadProject);
      setNativeState("ready");
      setNativeMessage(`${result.primitiveCount} primitives · ${result.jobId.slice(0, 8)}`);
      setNativeArtifacts(result.artifacts);
    } catch (error) {
      setNativeState("error");
      setNativeMessage(error instanceof Error ? error.message : "Không thể build native CAD artifacts.");
    }
  }
  return <div className="mech-page"><PageTitle eyebrow="PARAMETRIC ENCLOSURE" title="MECH" meta={`${cadProject.metrics.primitiveCount} PRIMITIVES`} /><div className="mech-toolbar"><div><button className={view === "iso" ? "active" : ""} onClick={() => setView("iso")}>ISO</button><button className={view === "top" ? "active" : ""} onClick={() => setView("top")}>TOP</button><button className={view === "front" ? "active" : ""} onClick={() => setView("front")}>FRONT</button></div><span>{cadProject.draftId.toUpperCase()} / V{version}</span><div><button className={exploded ? "active" : ""} onClick={() => setExploded((value) => !value)}>EXPLODE</button><button onClick={() => setResetToken((value) => value + 1)}>RESET</button></div></div><div className="mech-workspace">
    <div className="cad-stage"><CadViewport sceneSpec={cadProject.scene} view={view} exploded={exploded} resetToken={resetToken} /><div className="cad-runtime-badge"><i className={nativeState === "ready" || plasticityState === "ready" ? "connected" : nativeState === "error" && plasticityState === "error" ? "failed" : ""} /> DIY PREVIEW / NATIVE CAD <b>{cadProject.metrics.dimensionsMm.join(" × ")} MM</b></div><div className="dimension dim-width">{cadProject.metrics.dimensionsMm[0].toFixed(1)} MM</div></div>
    <aside className="feature-tree"><div className="feature-head"><span>FEATURE TREE</span><b>{cadProject.operations.length}</b></div>{cadProject.operations.map((operation, index) => <div className="feature" key={operation.id}><span>{String(index + 1).padStart(2, "0")}</span><i>◇</i><b>{operation.label}</b></div>)}<div className="mech-validation"><span>BUILD CONFIDENCE</span><b>{cadProject.validation.score}/100</b><div><i style={{ width: `${cadProject.validation.score}%` }} /></div><p>{cadProject.validation.checksPassed}/{cadProject.validation.checksTotal} checks passed</p></div><div className={`plasticity-runtime native ${nativeState}`}><span>DIY CAD · FREECAD/OCCT</span><b>{nativeState === "building" ? "BUILDING…" : nativeState === "ready" ? "ARTIFACTS READY" : nativeState === "error" ? "RUNTIME OFFLINE" : "LOCAL KERNEL"}</b><p>{nativeMessage}</p>{nativeArtifacts.length > 0 && <div className="native-artifacts">{nativeArtifacts.map((artifact) => <a href={artifact.url} key={artifact.name}>{artifact.kind.toUpperCase()}</a>)}</div>}</div><button className="commit-button native-build" disabled={nativeState === "building" || !cadProject.validation.passed} onClick={buildNativeCad}>{nativeState === "building" ? "BUILDING FCSTD / STEP / STL…" : nativeState === "ready" ? "REBUILD NATIVE CAD" : "BUILD WITH DIY CAD"}</button><div className={`plasticity-runtime ${plasticityState}`}><span>PLASTICITY RUNTIME</span><b>{plasticityState === "connecting" ? "CONNECTING…" : plasticityState === "ready" ? "CONNECTED" : plasticityState === "error" ? "OFFLINE" : "LOCAL BRIDGE"}</b><p>{plasticityMessage}</p></div><button className="commit-button" disabled={committed || plasticityState === "connecting" || !cadProject.validation.passed} onClick={onCommit}>{committed ? "OPENED IN PLASTICITY ✓" : plasticityState === "connecting" ? "SENDING CAD…" : "OPEN IN PLASTICITY"}</button></aside>
  </div></div>;
}
