...
export type VendorOption = { name: string; url: string; price: number; currency?: string; inStock?: boolean; note?: string };
...
export type ProjectPart = {
  ...
  sources?: VendorOption[];
};

// Nhiều lib/*-data.ts chỉ có purchaseUrl/amazonUrl/aliexpressUrl/ebayUrl rời rạc (chưa
// backfill field `sources`) — hàm này gộp chúng thành một danh sách VendorOption thống
// nhất để UI so giá dùng chung, không cần sửa dữ liệu 34 template mới chạy được.
export function partVendorOptions(part: ProjectPart): VendorOption[] {
  if (part.sources && part.sources.length > 0) return part.sources;
  const query = encodeURIComponent(part.productName);
  const fallback: Array<[string, string | undefined, boolean]> = [
    [part.sourceName || "Official", part.purchaseUrl, Boolean(part.purchaseUrl)],
    ["Amazon", part.amazonUrl || `https://www.amazon.com/s?k=${query}`, Boolean(part.amazonUrl)],
    ["AliExpress", part.aliexpressUrl || `https://www.aliexpress.com/wholesale?SearchText=${query}`, Boolean(part.aliexpressUrl)],
    ["eBay", part.ebayUrl || `https://www.ebay.com/sch/i.html?_nkw=${query}`, Boolean(part.ebayUrl)],
  ];
  return fallback
    .filter(([, url]) => Boolean(url))
    .map(([name, url, verified]) => ({ name, url: url as string, price: part.price, note: verified ? part.sourceStatus : "Đường dẫn tìm kiếm gợi ý" }));
}
